...
import { getSubjectChapters, subjectsInOrder } from '@/lib/db/subject-data'
...
export function CoachClient({ userName, targetPct: serverTargetPct, examDate }: CoachClientProps) {
  const selectedSubjectIds = useSubjectsStore((s) => s.selectedSubjectIds)
  const subjects = useMemo(() => subjectsInOrder(selectedSubjectIds), [selectedSubjectIds])
  const subjectNames = subjects.map((s) => s.name)

  // Use the goal store's target (kept in sync with DB via AppShell hydration).
  // Fall back to server-side value on first render before hydration.
  const storedTargetPct = useGoalStore((s) => s.targetPct)
  const targetPct = storedTargetPct ?? serverTargetPct

  const firstName = userName.split(' ')[0]

  const suggestions = useMemo(() => {
    const perSubject = subjects.slice(0, 3).map((s) => {
      const weakest = getSubjectChapters(s.id).slice().sort((a, b) => a.mastery_pct - b.mastery_pct)[0]
      return weakest
        ? `How should I revise ${weakest.name} in ${s.name}?`
        : `Give me a study tip for ${s.name}`
    })
    return [...perSubject, "I'm feeling overwhelmed — what should I prioritise?"]
  }, [subjects])

  const greeting = useMemo(
    () =>
      `Hi ${firstName}! I'm your AI learning coach for CBSE Class 12. I can help across your subjects — ${subjectNames.join(', ')}. What would you like to work on?`,
    [firstName, subjectNames]
  )

  const initialMessage: Message = {
    id: uid(),
    role: 'assistant',
    content: `Hi ${firstName}! I'm your AI learning coach. I know you're targeting **${targetPct}% overall** in your CBSE boards. How can I help you today?`,
  }

  const [conversations, setConversations] = useState<Conversation[]>(() => [
    { id: cid(), title: 'New chat', messages: [initialMessage] },
  ])
  ...
  function newConversation() {
    ...
    setConversations((prev) => [
      ...prev,
      { id, title: 'New chat', messages: [{ id: uid(), role: 'assistant', content: greeting }] },
    ])
    setActiveId(id)
